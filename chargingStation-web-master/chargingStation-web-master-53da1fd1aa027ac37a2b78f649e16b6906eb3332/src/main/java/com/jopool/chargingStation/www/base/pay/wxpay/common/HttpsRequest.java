...
import com.jopool.chargingStation.www.base.pay.wxpay.service.IServiceRequest;
...
public class HttpsRequest implements IServiceRequest {
    ...
    @Override
    public void init(WXAbstractConfig wxConfig) throws IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        this.wxConfig = wxConfig;
        KeyStore keyStore = KeyStore.getInstance("PKCS12");
        FileInputStream instream = new FileInputStream(new File(wxConfig.getCertLocalPath()));//加载本地的证书进行https加密传输
        try {
        ...
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, wxConfig.getCertPassword().toCharArray())
                .build();
