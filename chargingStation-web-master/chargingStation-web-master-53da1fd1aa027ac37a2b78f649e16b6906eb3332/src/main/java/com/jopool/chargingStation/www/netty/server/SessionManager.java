package com.jopool.chargingStation.www.netty.server;
...
public class SessionManager {
	...
	public synchronized Session put(String key, Session value) {
		if (value.getDeviceNumber() != null && !"".equals(value.getDeviceNumber().trim())) {
			this.phoneMap.put(value.getDeviceNumber(), value.getId());
		}
	...
	public synchronized Session removeBySessionId(String sessionId) {
		...
		if (session == null)
			return null;
		if (session.getDeviceNumber() != null)
			this.phoneMap.remove(session.getDeviceNumber());
		return session;
	}
