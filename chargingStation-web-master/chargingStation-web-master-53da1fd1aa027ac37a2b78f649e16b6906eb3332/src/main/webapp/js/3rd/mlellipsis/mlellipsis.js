...
Element.prototype.setHtml = function (str) {
...
Element.prototype.mlellipsis = function (row, showMore) {
    this.preRow = this.row;
    this.row = row;
    this.showMore = showMore === 'false' ? false : true;
    //省略后加上title
    ...
    if (height >= dheight) {
        ...
        while (dheight < this.clientHeight) {
            str = this.getText();
            this.setText(str.replace(/(\s)*([a-zA-Z0-9]?|\W|_)(\.\.\.)?$/, "..."));
        }
        var moreTxt = that.showMore ? '<b class="_mlellipsis_more_">展开</b>' : '';
        this.setHtml(this.getText().substring(0, str.length - 10) + "..." + moreTxt);
        $(this).find('._mlellipsis_more_').unbind().click(function (e) {
            $(this).parent()[0].mlellipsis(99999);
            $(this).hide();
            e.stopPropagation();
        });
    } else if (row == 99999) {
        var lessTxt = that.showMore ? '<b class="_mlellipsis_less_">隐藏</b>' : '';
        this.setHtml(this.getText() + lessTxt);
        $(this).find('._mlellipsis_less_').unbind().click(function (e) {
            $(this).parent()[0].mlellipsis(that.preRow);
            $(this).hide();
            e.stopPropagation();
        });
    }
