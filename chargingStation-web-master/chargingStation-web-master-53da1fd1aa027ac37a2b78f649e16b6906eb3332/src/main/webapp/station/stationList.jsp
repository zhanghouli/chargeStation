...
<style>
    .selectnone .uew-select .ue-state-default {
        display: none;
    }
    .select2-container{
        z-index: 9999;
    ...
    <ul class="seachform">
        <form method="get" action="" id="searchForm">
            ...
            <li>
                ...
                <div class="vocation">
                    <select name="status">
                        ...
                        <c:forEach var="s" items="${STATION_STATUS}">
            ...
            <li><label>关键字</label>
                ...
                <input name="" type="button" class="scbtn _searchBtn" value="查询" action="stationList.htm"/>
            </li>
        </form>
    </ul>
    ...
    <table class="tablelist">
        ...
        <c:forEach var="i" items="${list}">
            <tr data="${i.id}">
                ...
                <td>
                    <a href="${path}/station/stationDetails.htm?stationId=${i.id}&estateId=${estateId}&operatorId=${operatorId}&stationType=normal"
                    ...
                    <br>
                    <a href="${path}/station/stationPortList.htm?stationId=${i.id}&estateId=${estateId}&operatorId=${operatorId}&stationType=normal"
                       class="tablelink _selectStationPor">充电口管理</a>&nbsp;
                    <a href="${path}/station/stationMaintainLogList.htm?stationId=${i.id}&estateId=${estateId}&operatorId=${operatorId}&stationType=normal"
                       class="tablelink ">电站维护</a>&nbsp;
                    <a href="javaScript:void(0);" data-name="${i.name}"
                       class="tablelink _estateModify">物业更换</a>&nbsp;
...
<!--modify eatate-->
<ul class="seachform zxxbox_contaner validationEngineContainer" id="replaceEstate" style="width:350px">
    <input type="hidden" value="" name="estateStationId">
    <li class="line">
        <label>物业名称:</label>
        <div class="vocation">
            <select name="estateName" class=" select2  validate[required]" multiple="multiple">
            </select>
        </div>
    </li>
    <li class="line">
        <label>支付套餐Id:</label>
        <div class="vocation">
            <select width="130" id="estateConsumePackageId" name="estateConsumePackageId" class="validate[required]">
                <option value="">请选择</option>
            </select>(没有套餐请与此相关联运营商增加套餐)
        </div>
    </li>
    <li class="line" style="text-align: center;width: 100%;">
        <input name="" type="button" class="btn replaceEstateSave" value="确认"/>
    </li>
</ul>
...
<script>
    $(function () {
        ...
        //物业替换
        $('._estateModify').on("click", function () {
            var title = "(电站名称:" + $(this).attr("data-name") + ")";
            $('input[name="estateStationId"]').val($(this).parents('tr').attr("data"));
            $K.http("${path}/station/getAllEstate.htm", {}, function (r) {
                $('select[name="estateName"]').empty();
                $('select[name="estateConsumePackageId"]').empty();
                $('select[name="estateConsumePackageId"]').append('<option value="">请选择</option>');
                var html = "", estateList = r.result;
                for (var i = 0; i < estateList.length; i++) {
                    html += '<option data-passportId=' + estateList[i].passportId + ' data-operator=' + estateList[i].operatorId + ' value=' + estateList[i].estateId + '>' + estateList[i].realName + '</option>'
                }
                $('select[name="estateName"]').append(html);
                $.zxxbox($('#replaceEstate'), {
                    title: "物业替换" + title
                });
        ...
        //物业选择
        $('select[name="estateName"]').on("change", function () {
            if (this.value.length > 0) {
                var _that = $('select[name="estateName"] :selected');
                $K.http('${path}/passport/getOperatorId.htm',
                    {
                        operatorId: _that.attr("data-operator")
                    }, function (r) {
                        $("#estateConsumePackageId").empty();
                        var operator = r.result;
                        var map = operator.maps;
                        $("#estateConsumePackageId").append("<option value=''>请选择</option>");
                        if (map) {
                            $.each(map, function (index, element) {
                                $("#estateConsumePackageId").append("<option value=" + index + ">" + element + "</option>");
                            })
                        }
                    });
            }
        ...
        //提交替换物业
        $('.replaceEstateSave').ajaxbtn("modifyStationReplaceEstate.htm", function () {
            return {
                stationId: $('input[name="estateStationId"]').val(),
                estateId: $('select[name="estateName"]').val()[0],
                consumePackageId: $('select[name="estateConsumePackageId"]').val()
            }
        }, function () {
            return $('#replaceEstate').validationEngine('validate', {validateNonVisibleFields: true});
        })
        //多选
        $('select[name="estateName"]').select2({
            language: "zh-CN",
            maximumSelectionLength: 1
        });
        //隐藏
        $('select[name="estateName"]').prev().hide();
        //map
        //@@@@ map
